...
const Home = () => {
	...
	const handleCardClick = async (influencer: InfluencerSummary) => {
		try {
			const response = await fetch(
				`http://backen-production-8389.up.railway.app/influencer/${influencer.id}`
			);
			if (response.ok) {
				...
				try {
					const videoResponse = await fetch(
						`http://backen-production-8389.up.railway.app/influencer/${influencer.id}/videos`
					);
					if (videoResponse.ok) {
	...
	const handleFetchInfluencers = async () => {
		try {
			const response = await fetch(
				"http://backen-production-8389.up.railway.app/influencers?skip=0&limit=100"
			);
			if (response.ok) {
