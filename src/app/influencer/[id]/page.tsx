...
const InfluencerProfilePage = () => {
	...
	const fetchInfluencerData = useCallback(async () => {
		...
		try {
			const influencerRes = await fetch(
				`http://backen-production-8389.up.railway.app/influencer/${influencerId}`
			);
			if (!influencerRes.ok) {
			...
			const videoRes = await fetch(
				`http://backen-production-8389.up.railway.app/influencer/${influencerId}/videos`
			);
			let videos: Video[] = [];
			if (videoRes.ok) {
	...
	const handleScheduleSubmit = async (postData: any) => {
		...
		try {
			const response = await fetch("http://backen-production-8389.up.railway.app/schedule", {
				method: "POST",
